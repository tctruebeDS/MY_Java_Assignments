...
// Theodore Truebe
// Unit 05 HW 13 Part C
import java.awt.Color;
import java.awt.Graphics;
import javax.swing.*;

public class HW13_PartC {
    /*
     * Change the following method to customize
     * what is drawn in the JFrame.
     */
    static int scale = 150;

    public static void draw(Graphics g) {
        final double TWOPI = 2 * Math.PI;
        final double RESOLUTION = 1000;
        double delta= (TWOPI) / RESOLUTION;
        double theta = 0; 
        
        while (theta <= TWOPI) {
            double r = Math.cos(2* theta);
            int xVal = (int)(100*r  * Math.cos(theta));         
            int yVal = (int)(100*r  * Math.sin(theta));    
            xVal= xVal + 200;
            yVal= yVal + 200;
            g.setColor(Color.BLUE);
            g.fillOval(xVal, yVal, 2, 2);
            theta += delta;
            /* r=hyp, theta = angle
            *costheta = adj/hyp, so x = r*costheta
            *theta=independant, from 0 to 2pi rad
            */
        }
